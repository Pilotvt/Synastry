import { OVERLAY_RULES, type OverlayRule, type Planet as OverlayPlanet } from "./overlayRules";
...
function scoreNodes(left: ChartPayload, right: ChartPayload) {
  // Heuristic: Rahu overlay to partner's houses
  const la = getAscSignCode(right); // to overlay A's nodes onto RIGHT houses
  const lb = getAscSignCode(left);  // overlay B's nodes onto LEFT houses
  const notes: string[] = [];
  let sum = 0; let cnt = 0;
  if (la) {
    const aRa = getPlanetSignCode(left, "Ra");
    const aKe = getPlanetSignCode(left, "Ke");
    if (aRa) { const h = signToHouse(la, aRa); const r = nodeHouseVal(h); sum += r; cnt++; notes.push(`Раху1→дом2=${h}: ${fmt(r)}`); }
    if (aKe) { const h = signToHouse(la, aKe); const r = -nodeHouseVal(h) * 0.6; sum += r; cnt++; notes.push(`Кету1→дом2=${h}: ${fmt(r)}`); }
  }
  if (lb) {
    const bRa = getPlanetSignCode(right, "Ra");
    const bKe = getPlanetSignCode(right, "Ke");
    if (bRa) { const h = signToHouse(lb, bRa); const r = nodeHouseVal(h); sum += r; cnt++; notes.push(`Раху2→дом1=${h}: ${fmt(r)}`); }
    if (bKe) { const h = signToHouse(lb, bKe); const r = -nodeHouseVal(h) * 0.6; sum += r; cnt++; notes.push(`Кету2→дом1=${h}: ${fmt(r)}`); }
  }
  const raw = cnt ? clamp(sum / cnt, -1, 1) : 0;
  return { raw, details: notes };
}

function nodeHouseVal(h: number): number {
  if (h === 10 || h === 11) return 0.8;
  if (h === 1 || h === 7) return 0.6;
  if (h === 5 || h === 9) return 0.35;
  if (h === 4 || h === 8 || h === 12) return -0.6;
  return 0.0;
}
...
function collectPlanetSigns(chart: ChartPayload): Array<[PlanetCode, SignCode]> {
  const out: Array<[PlanetCode, SignCode]> = [];
  const tryCollect = (obj: Record<string, unknown>) => {
    const planets = (obj as any).planets;
    if (!Array.isArray(planets)) return;
    for (const p of planets) {
      if (!isRecord(p) || typeof (p as any).name !== 'string' || typeof (p as any).sign !== 'string') continue;
      const name = (p as any).name as string;
      const sign = (p as any).sign as string;
      if ((["Su","Mo","Ma","Me","Ju","Ve","Sa","Ra","Ke"] as string[]).includes(name) && sign in SIGN_INDEX) {
        out.push([name as PlanetCode, sign as SignCode]);
      }
    }
  };
  if (isRecord(chart)) tryCollect(chart);
