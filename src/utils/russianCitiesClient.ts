...
function normalizeRussianCity(raw: any): RussianCityRecord | null {
  if (!raw || typeof raw !== "object") return null;
  const name = typeof raw.name === "string" ? raw.name.trim() : "";
  if (!name) return null;
  const latRaw = raw.coords?.lat ?? raw.lat ?? raw.latitude;
  const lonRaw = raw.coords?.lon ?? raw.lon ?? raw.longitude;
