...
import { supabase } from "../lib/supabase"; // нужен только для проверки лицензии/сессии
...
const CLOUD_DISABLED = true; // Полностью локальная работа на странице "Дополнительно"
...
const AdditionalChartPage = () => {
  ...
  if (!skipLocalCache) {
        ...
        const cloudSnapshot = CLOUD_DISABLED ? null : extractProfileSnapshot(profileRow?.data ?? null);
      ...
      let snapshot = skipLocalCache
        ? (pickDirectSnapshot(localSnapshotOnly)
            ...
            ?? pickDirectSnapshot(CLOUD_DISABLED ? null : cloudSnapshot))
        : pickFreshProfile(localSnapshotOnly, fallbackProfile, CLOUD_DISABLED ? null : cloudSnapshot);
        if (snapshot) {
        ...
        // Cloud sync отключен в Additional
...
const daraKarakaBody = daraKarakaDescriptionParts.body || (!daraKarakaDescriptionParts.heading ? daraKarakaDescription : "");
  ...
  // В дополнительной странице облако не используется
  const uploadChartScreenshot = useCallback(async (): Promise<boolean> => false, []);
